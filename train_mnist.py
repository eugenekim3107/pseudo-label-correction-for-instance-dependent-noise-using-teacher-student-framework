...
def main():
    ...
    targets_noisy = torch.Tensor(pd.read_csv(os.path.join('./data/MNIST/label_noisy', 'dependent' + str(args.noise_rate)+'.csv'))['label_noisy'].values.astype(int))
    ...
    corrected_dataset = SiameseMNISTCorrectionDataset(meta_dataset, base_dataset, student_model, teacher_model, args.seed)
...
#     base_dataset.targets = torch.Tensor(y_corrected).type(torch.int)
    ...
    train_loader = DataLoader(base_dataset, batch_size = args.batch_size, shuffle=False, **kwargs)
    torch.save(base_dataset, f"data/MNIST/final_dataset{args.noise_rate}.pth")
    torch.save(torch.sum(y_noisy != y_corrected) / y_noisy.shape[0], f"result/mnist_noise_estimate_{args.noise_rate}_{args.seed}.pth")
    ...
    print(test_losses)
    print(test_accs)
    torch.save(torch.Tensor(test_accs), f"result/mnist_test_acc_{args.noise_rate}_{args.seed}.pth")
