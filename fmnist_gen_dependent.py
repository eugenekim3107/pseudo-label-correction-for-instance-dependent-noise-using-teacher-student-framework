...
def main():
    ...
    transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.2860,), (0.3530,))]) #0.2860, 0.3530 are the mean and std of fashion mnist
    ...
    if args.load:
        softmax_out_avg = np.load('data/FashionMNIST/label_noisy/softmax_out_avg.npy')
        ...
        np.save('data/FashionMNIST/label_noisy/softmax_out_avg.npy', softmax_out_avg)
    ...
    if args.gen:
        ...
        save_pth = os.path.join('./data/FashionMNIST/label_noisy', 'dependent'+str(args.noise_rate)+'.csv')
